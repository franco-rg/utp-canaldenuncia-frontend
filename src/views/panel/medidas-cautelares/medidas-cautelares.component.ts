...
@Component({
  selector: 'app-medidas-cautelares',
  templateUrl: './medidas-cautelares.component.html',
  styleUrls: ['./medidas-cautelares.component.css'],
})
export class MedidasCautelaresComponent {
  data: ReceptorModel[] = [];
  ...
  onPageChange(page: number | null = null) {
    ...
    this.loading = true;
    this.canalAdminService.getMedidasCautelares(
      ...
      this.canalAdminService.postDeleteConfiguracionDenuncia(id).subscribe({
        next: (res) => {
          if (res.codEstado > 0 && res.esSatisfactoria) {
            this.notyf.success('Medida Cautelar cambiado de estado correctamente');
            this.onPageChange();
          } else {
            this.notyf.error('Error al cambiar el estado la Medida Cautelar');
          }
        },
        error: () => {
          this.notyf.error('Error al cambiar el estado la Medida Cautelar');
