...
@Component({
  selector: 'app-modal-medida-cautelar-editar',
  templateUrl: './modal-medida-cautelar-editar.component.html',
  styleUrls: ['./modal-medida-cautelar-editar.component.css'],
})
export class ModalMedidaCautelarEditarComponent implements OnInit {
  @Input() show = false;
    ...
    this.canalAdminService.postEditConfiguracionDenuncia(formData).subscribe({
      next: (res) => {
        if (res.codEstado > 0 && res.esSatisfactoria) {
          this.notyf.success('Medida Cautelar Editado correctamente');
          this.reload.emit(null);
          this.cerrar();
        } else {
          this.notyf.error('Error al editar la Medida Cautelar');
        }
      },
      error: (err) => {
        this.notyf.error('Error al editar la Medida Cautelar');
