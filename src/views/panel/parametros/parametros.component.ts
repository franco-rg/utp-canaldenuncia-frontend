...
export class ParametrosComponent {
  ...
  displayedColumns: any[] = [
    ...
    { title: "Abreviatura", style: "width: 20%", class: "text-left", html: false, keyof: "abrev" },
    { title: "Valor 1", style: "width: 20%", class: "text-left", html: false, keyof: "vlR1" },
    { title: "Valor 2", style: "width: 20%", class: "text-left", html: false, keyof: "vlR2" },
    { title: "Valor 3", style: "width: 20%", class: "text-left", html: false, keyof: "vlR3" },
  ...
  onPageChange(page: number | null = null) {
    ...
    this.loading = true;
    this.canalAdminService.getParametros(
  ...
  eliminar(id: number) {
    ...
    this.notyf.swalDeleteConfirm(() => {
      this.loading = true;
      this.canalAdminService.postDeleteParametro(id).subscribe({
        next: (res) => {
          if (res.codEstado > 0 && res.esSatisfactoria) {
            this.notyf.success('Parametro cambiado de estado correctamente');
            this.onPageChange();
          } else {
            this.notyf.error('Error al cambiar el estado del Parametro');
          }
        },
        error: () => {
          this.notyf.error('Error al cambiar el estado del Parametro');
