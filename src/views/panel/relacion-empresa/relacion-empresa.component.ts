...
@Component({
  selector: 'app-relacion-empresa',
  templateUrl: './relacion-empresa.component.html',
  styleUrls: ['./relacion-empresa.component.css'],
})
export class RelacionEmpresaComponent {
  data: ReceptorModel[] = [];
  ...
  onPageChange(page: number | null = null) {
    ...
    this.loading = true;
    this.canalAdminService.getRelacionEmpresa(
      ...
      this.canalAdminService.postDeleteConfiguracionDenuncia(id).subscribe({
        next: (res) => {
          if (res.codEstado > 0 && res.esSatisfactoria) {
            this.notyf.success('Relación con empresa cambiado de estado correctamente');
            this.onPageChange();
          } else {
            this.notyf.error('Error al cambiar el estado la Relación con empresa');
          }
        },
        error: () => {
          this.notyf.error('Error al cambiar el estado la Relación con empresa');
