...
@Component({
  selector: 'app-modal-rol-gestor-editar',
  templateUrl: './modal-rol-gestor-editar.component.html',
  styleUrls: ['./modal-rol-gestor-editar.component.css'],
})
export class ModalRolGestorEditarComponent implements OnInit {
  @Input() show = false;
    ...
    this.canalAdminService.postEditConfiguracionDenuncia(formData).subscribe({
      next: (res) => {
        if (res.codEstado > 0 && res.esSatisfactoria) {
          this.notyf.success('Rol Editado correctamente');
          this.reload.emit(null);
          this.cerrar();
        } else {
          this.notyf.error('Error al editar el rol');
        }
      },
      error: (err) => {
        this.notyf.error('Error al editar el rol');
