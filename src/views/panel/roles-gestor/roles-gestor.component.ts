...
@Component({
  selector: 'app-roles-gestor',
  templateUrl: './roles-gestor.component.html',
  styleUrls: ['./roles-gestor.component.css'],
})
export class RolesGestorComponent {
  data: ReceptorModel[] = [];
  ...
  eliminar(id: number) {
    if (!id) return this.notyf.error('Error al cambiar el estado del rol');
      ...
      this.canalAdminService.postDeleteConfiguracionDenuncia(id).subscribe({
        next: (res) => {
          if (res.codEstado > 0 && res.esSatisfactoria) {
            this.notyf.success('Rol cambiado de estado correctamente');
            this.onPageChange();
          } else {
            this.notyf.error('Error al cambiar el estado del rol');
          }
        },
        error: () => {
          this.notyf.error('Error al cambiar el estado del rol');
