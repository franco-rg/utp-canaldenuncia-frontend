...
@Component({
  selector: 'app-modal-tipo-denuncia-agregar',
  templateUrl: './modal-tipo-denuncia-agregar.component.html',
  styleUrls: ['./modal-tipo-denuncia-agregar.component.css'],
})
export class ModalTipoDenunciaAgregarComponent implements OnInit {
  @Input() show = false;
    ...
    this.canalAdminService.postAddConfiguracionDenuncia(formData).subscribe({
      next: (res) => {
        if (res.codEstado > 0 && res.esSatisfactoria) {
          this.notyf.success('Tipo Denuncia agregado correctamente');
          this.reload.emit(null);
          this.cerrar();
        } else {
          this.notyf.error('Error al agregar el tipo denuncia');
        }
      },
      error: () => {
        this.notyf.error('Error al agregar el tipo denuncia');
