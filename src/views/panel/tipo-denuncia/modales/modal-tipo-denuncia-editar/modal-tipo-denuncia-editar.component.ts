...
@Component({
  selector: 'app-modal-tipo-denuncia-editar',
  templateUrl: './modal-tipo-denuncia-editar.component.html',
  styleUrls: ['./modal-tipo-denuncia-editar.component.css'],
})
export class ModalTipoDenunciaEditarComponent implements OnInit {
  @Input() show = false;
    ...
    this.canalAdminService.postEditConfiguracionDenuncia(formData).subscribe({
      next: (res) => {
        if (res.codEstado > 0 && res.esSatisfactoria) {
          this.notyf.success('Tipo denuncia Editado correctamente');
          this.reload.emit(null);
          this.cerrar();
        } else {
          this.notyf.error('Error al editar el tipo denuncia');
        }
      },
      error: (err) => {
        this.notyf.error('Error al editar el tipo denuncia');
